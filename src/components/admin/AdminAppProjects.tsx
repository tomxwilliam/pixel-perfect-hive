...
import { useAppProjects, useUpdateAppProject, useDeleteAppProject, AppProject } from "@/hooks/useAppProjects";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent } from "@/components/ui/card";
import { Plus, Search, Edit, Trash2, GripVertical, Smartphone, Star } from "lucide-react";
import { FaApple, FaGooglePlay } from "react-icons/fa";
import { CreateEditAppProjectDialog } from "./forms/CreateEditAppProjectDialog";
import { ConfirmDeleteDialog } from "@/components/ui/confirm-delete-dialog";

export function AdminAppProjects() {
  ...
  const handleDelete = (project: AppProject) => {
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold">App Projects</h1>
          <p className="text-muted-foreground">Manage your mobile app portfolio</p>
        </div>
          ...
          <Card key={project.id}>
            <CardContent className="p-4 space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Badge variant="outline">{project.app_category}</Badge>
                {project.is_featured && <Badge variant="secondary"><Star className="h-3 w-3" /></Badge>}
                {project.ios_link && <FaApple className="h-4 w-4" />}
                {project.android_link && <FaGooglePlay className="h-4 w-4" />}
              </div>
