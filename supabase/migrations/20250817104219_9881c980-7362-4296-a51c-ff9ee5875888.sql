...
-- Update existing tickets with ticket numbers using a different approach
DO $$
DECLARE
  ticket_record RECORD;
  counter INTEGER := 1;
BEGIN
  FOR ticket_record IN SELECT id FROM tickets WHERE ticket_number IS NULL ORDER BY created_at LOOP
    UPDATE tickets SET ticket_number = counter WHERE id = ticket_record.id;
    counter := counter + 1;
  END LOOP;
END $$;